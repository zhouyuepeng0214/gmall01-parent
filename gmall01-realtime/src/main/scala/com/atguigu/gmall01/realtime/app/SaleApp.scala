...
import com.atguigu.gmall01.realtime.bean.{OrderDetail, OrderInfo, SaleDetail, UserInfo}
...
object SaleApp {
  def main(args: Array[String]): Unit = {
    ...
    val userRecordDstream: InputDStream[ConsumerRecord[String, String]] = MyKafkaUtil.getKafkaStream(GmallConstants.KAFKA_TOPIC_USER, ssc)
    ...
    //            }
    //
    //        orderDetailRecordDstream.foreachRDD{rdd =>
    //              println(rdd.map(_.value()).collect().mkString("\n"))
    //            }

    //    userRecordDstream.foreachRDD { rdd =>
    //      println(rdd.map(_.value()).collect().mkString("\n"))
    //    }
    ...
    val orderDetailWithKeyDstream: DStream[(String, OrderDetail)] = orderDetailDstream.map { orderDetail =>
    ...
    val saleDetailDstream: DStream[SaleDetail] = fulljoinDstream.flatMap {
      case (orderId, (orderInfoOpt, orderDetailOpt)) => {
        ...
        val jedis = new Jedis("hadoop110", 6379)
...
//        println(dataList)
        //        MyEsUtil.indexBUlk(GmallConstants.ES_INDEX_ALTER, dataList)
      }
    }
